...
/*
   ...
   */
function m(e,i){var r="fjs-"+e,n=t("["+r+"]"),s=[];if(n.length)return t.each(n,function(){var e=t(this),i={ele:e},n=e.attr(r);if(s.push(i),!n)return s;t.each(n.split(","),function(t,e){var r=e.split("=");i[r[0]]=r[1]})}),i?s:s[0]}v.list=g,v.templateBuilder=n,e.FilterJS=v,d.pagination='<nav>  <ul class="pagination">    <% if(currentPage > 1) { %>      <li> <a href="#" data-page="first" aria-label="First"><span aria-hidden="true">Primero</span></a> </li>      <li><a href="#" data-page="prev" aria-label="Previous"><span aria-hidden="true">&larr; Anterior</span></a></li>    <% } %>    <% for(var i = 0, l = pages.length; i < l; i++ ){ %>      <li class="<%= pages[i] == currentPage ? \'active\' : \'\' %>">        <a href="#" data-page="<%= pages[i] %>"><%= pages[i] %></a>      </li>    <% } %>    <% if( currentPage < totalPages ) { %>      <li><a href="#" data-page="next" aria-label="Next"><span aria-hidden="true">Siguiente &rarr;</span></a></li>      <li><a href="#" data-page="last" aria-label="Last"><span aria-hidden="true">Último</span></a></li>    <% } %>  </ul></nav>',d.per_page='<select size="1" name="per_page" data-perpage="true" class="per-page">  <% for(var i = 0; i < values.length; i++ ){ %>    <option value="<%= values[i] %>"><%= values[i] %></option>  <% } %></select>',v.auto=function(t,e){var i,r,n,s={},a=m("items");if(a&&a.template)return s.template=a.template,(r=m("search"))&&(r.fields&&(r.fields=r.fields.split(",")),s.search=r),e&&(s.callbacks=e),i=v(t,a.ele,s),(n=m("criteria",!0))&&i.addCriteria(n),i}}(jQuery,window,document);